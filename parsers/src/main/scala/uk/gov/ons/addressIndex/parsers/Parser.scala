...
object Parser {
  ...
  // todo look at replacing deprecated mapValues, e.g with .groupMap see https://www.scala-lang.org/blog/2018/06/13/scala-213-collections.html
    val groupedTokens: Map[String, Array[String]] =
      labels
        ...
        .mapValues(_.map { case (_, token) => token }).toMap // finally we have Array( (label1, Array(token1.1, token1.2)), (label2, Array(token2)) )
