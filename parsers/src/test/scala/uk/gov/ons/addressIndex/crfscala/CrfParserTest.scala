...
class CrfParserTest extends FlatSpec with Matchers {
  ...
  it should "return the expected `CrfJniInput` for a combination of feature analysers (String, Boolean, Double, Int - (all supported types)) for `CrfTokens` size multiple" in {
    val token1 = "token1"
    val token2 = "token2"
    val token3 = "token3"
    ...
    val token3Expectedf4 = s"\t${feature4.name}:7.0\tprevious\\:${feature4.name}:7.0\n"
    val expected =
      token1Expectedf1 + token1Expectedf2 + token1Expectedf3 + token1Expectedf4 +
      token2Expectedf1 + token2Expectedf2 + token2Expectedf3 + token2Expectedf4 +
