...
class RegistersEQCustomSimulation extends Simulation {
  ...
  val feeder: BatchableFeederBuilder[String] = csv("postcodes100K.csv").random
  ...
  scenario(requestName)
    ...
    .feed(feeder)
    .exec(http("Postcode")
      .get(requestRelPath + "?input=" + "${postcode}")
    )
