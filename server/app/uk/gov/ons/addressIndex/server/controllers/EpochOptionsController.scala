...
import uk.gov.ons.addressIndex.model.server.response.address.{AddressResponseEpoch, AddressResponseVersion}
...
class EpochOptionsController @Inject()(val controllerComponents: ControllerComponents,
  ...
  lazy val epochResults = new AddressResponseEpoch(epochList, epochDates)
