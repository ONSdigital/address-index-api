...
class RelativesExpanderTest extends FlatSpec with Matchers with ScalaFutures {
  "RelativesExpander" should
    ...
    val conf: DemouiConfigModule = app.injector.instanceOf[DemouiConfigModule]
    ...
    val rels: Seq[AddressResponseRelative] = Seq(rel1, rel2)

    // Given
    val esib1 = ExpandedSibling(1, "7, GATE REACH, EXETER, EX2 9GA")
    val esib2 = ExpandedSibling(2, "7, GATE REACH, EXETER, EX2 9GA")
    val esib3 = ExpandedSibling(3, "7, GATE REACH, EXETER, EX2 9GA")
    val ex1 = ExpandedRelative(1, Seq(esib1))
    val ex2 = ExpandedRelative(2, Seq(esib2, esib3))
    val expectedSeq: Seq[ExpandedRelative] = Seq(ex1, ex2)
